...
const MessageBoxContext = createContext<MessageBoxContextExposes>(
  {} as MessageBoxContextExposes
);
...
export const MessageBoxProvider = ({
  ...
  const openModal = useCallback(
    ({ classNames, ...modalContent }: Omit<ModalProps, "id">) => {
  ...
  return (
    <MessageBoxContext.Provider value={exposes}>
      {children}
      {modals.map(
        ({ id, resolve, reject, header, body, footer, close, ...rest }) => (
          <Modal
            {...rest}
            key={id}
            isOpen={true}
